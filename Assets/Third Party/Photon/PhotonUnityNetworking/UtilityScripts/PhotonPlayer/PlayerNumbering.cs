...
namespace Photon.Pun.UtilityScripts
{
    ...
    public class PlayerNumbering : MonoBehaviourPunCallbacks
    {
        ...
        public static PhotonNetworkedPlayer[] SortedPlayers;
        ...
        public override void OnPlayerLeftRoom(PhotonNetworkedPlayer otherPlayer)
        {
            this.RefreshData();
        }

        public override void OnPlayerPropertiesUpdate(PhotonNetworkedPlayer targetPlayer, Hashtable changedProps)
        {
       ...
       public void RefreshData()
        {
            ...
            PhotonNetworkedPlayer[] sorted = PhotonNetwork.PlayerList.OrderBy((p) => p.ActorNumber).ToArray();

            string allPlayers = "all players: ";
            foreach (PhotonNetworkedPlayer player in sorted)
            {
    ...
    public static class PlayerNumberingExtensions
    {
        ...
        public static int GetPlayerNumber(this PhotonNetworkedPlayer player)
        {
			if (player == null) {
				return -1;
			}
        ...
        public static void SetPlayerNumber(this PhotonNetworkedPlayer player, int playerNumber)
        {
			if (player == null) {
				return;
			}
