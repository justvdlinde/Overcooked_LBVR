...
namespace Photon.Pun
{
    ...
    public static partial class PhotonNetwork
    {
        ...
        public static Dictionary<int, PhotonView> PhotonViewList => photonViewList;
        ...
        private static event Action<PhotonView, PhotonNetworkedPlayer> OnOwnershipRequestEv;
        /// <summary>Parameters: PhotonView for which ownership was requested, player who requests ownership.</summary>
        private static event Action<PhotonView, PhotonNetworkedPlayer> OnOwnershipTransferedEv;
        ...
        /// </summary>
        internal static void ExecuteRpc(Hashtable rpcData, PhotonNetworkedPlayer sender)
        {
        ...
        public static void RegisterPhotonView(PhotonView netView)
        {
            ...
            if (isViewListed)
            {
                ...
                {
                    // Changed this to replace first copy instead of removing the actual duplicate
                    LocalCleanPhotonView(netView);
        ...
        internal static void RPC(PhotonView view, string methodName, RpcTarget target, PhotonNetworkedPlayer player, bool encrypt, params object[] parameters)
        {
            ...
            switch (target)
            {
                ...
                case RpcTarget.AllGlobalBuffered:
                {
                    RaiseEventOptions options = new RaiseEventOptions() { CachingOption = EventCaching.AddToRoomCacheGlobal };
                    PhotonNetwork.RaiseEventInternal(PunEvent.RPC, rpcEvent, options, sendOptions);
                ...
                case RpcTarget.OthersGlobalBuffered:
                {
                    RaiseEventOptions options = new RaiseEventOptions() { CachingOption = EventCaching.AddToRoomCacheGlobal };
                    PhotonNetwork.RaiseEventInternal(PunEvent.RPC, rpcEvent, options, sendOptions);
                    break;
    ...
    private class SerializeViewBatch : IEquatable<SerializeViewBatch>, IEquatable<RaiseEventBatch>
    {
        ...
        /// </summary>
        private static void OnSerializeRead(object[] data, PhotonNetworkedPlayer sender, int networkTime, short correctPrefix)
        {
        ...
        private static void OnEvent(EventData photonEvent)
        {
            ...
            PhotonNetworkedPlayer originatingPlayer = null;
                    ...
                    switch (requestedView.OwnershipTransfer)
                    {
                        case OwnershipOption.Takeover:
                            ...
                            if (requestedFromOwnerId == currentPvOwnerId || (requestedFromOwnerId == 0 && currentPvOwnerId == MasterClient.ActorNumber) || currentPvOwnerId == 0)
                            {
                                ...
                                PhotonNetworkedPlayer previousOwner = CurrentRoom.GetPlayer(currentPvOwnerId);
